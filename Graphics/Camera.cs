...
namespace Graphics
{
    class Camera
    {
        ...
        public Camera()
        {
            ...
            SetProjectionMatrix(45, 4 / 3, 0.1f, 12000);
        }
        ...
        {
            if ((mCenter + dist * mDirection).z < 6900 && 
                (mCenter + dist * mDirection).z > -6900)
                mCenter += dist * mDirection;
        }
        public void Strafe(float dist)
        {
            if ((mCenter + dist * mRight).x < 6900 && (mCenter + dist * mRight).x > -6900)
                mCenter += dist * mRight;
        }
        public void Fly(float dist)
        {
            if ((mCenter + dist * mUp).y < 10900 && (mCenter + dist * mUp).y > 10)
                 mCenter += dist * mUp;
        }
    }
}
